...
xss_payload_pattern = re.compile(r'</?(?:script|textarea|form|marquee|iframe|frame|xss|var|object|xml|button|embed|font|select|div|meta|br|label|set|img|image|head|audio|video|svg|title|input|circle|body|p|html|style|a|isindex|x|sc|foo|object|link)|prompt|alert|javascript:|xss\.nasl', flags=re.IGNORECASE)
...
#sql_injection_payload_pattern = re.compile(r"\b(or|and)\b\s+\d+|\b(or|and)\b\s+(@@version|EXISTS)|\bselect\b\s+(?:\*|\w+)\s+\bfrom\b|\bselect\s+\*|\binsert\b\s+into\b|\bunion\b\s+(?:all\s+)?\bselect\b|\bconcat\b|\border\b\s+by\b|\bgroup\b\s+by\b|\bdelete\b\s+\bfrom\b|union\s+select|union\W+select|\bdrop\b\s+(?:table|database)\b|\bwaitfor\b\s+delay\b|\bbenchmark\b|\b(if)\b\s+\(.+?=.+?\)|\blike\b\s+'.*?'|\b(?:OR|AND)\b\s+(?:(?:(?:\d+\s*[=<>]+\s*\d+)|(?:[a-z_]+\s*[=<>]+\s*[a-z_]+))|(?:\w+\s*=\s*[']?\w+[']?))|['\";]+--.*", flags=re.IGNORECASE)
#sql_injection_payload_pattern = re.compile(r"\b(or|and)\b\s+\d+|\b(or|and)\b\s+(@@version|EXISTS)|\bselect\b\s+(?:\*|\w+)\s+\bfrom\b|\bselect\s+\*|\binsert\b\s+into\b|\bunion\b\s+(?:all\s+)?\bselect\b|\bconcat\b|\border\b\s+by\b|\bgroup\b\s+by\b|\bdelete\b\s+\bfrom\b|union\s+select|union\W+select|\bdrop\b\s+(?:table|database)\b|\bwaitfor\b\s+delay\b|\bbenchmark\b|\b(if)\b\s+\(.+?=.+?\)|\blike\b\s+'.*?'|\b(?:OR|AND)\b\s+(?:(?:(?:\d+\s*[=<>]+\s*\d+)|(?:[a-z_]+\s*[=<>]+\s*[a-z_]+))|(?:\w+\s*=\s*[']?\w+[']?))|['\";]+--.*", flags=re.IGNORECASE)
sql_injection_payload_pattern = re.compile(r"\b(or|and)\b\s+\d+|\b(or|and)\b\s+(@@version|EXISTS)|\bselect\b\s+(?:\*|\w+)\s+\bfrom\b|\bselect\s+\*|\binsert\b\s+into\b|\bunion\b\s+(?:all\s+)?\bselect\b|\bconcat\b|\border\b\s+by\b|\bgroup\b\s+by\b|\bdelete\b\s+\bfrom\b|union\s+select|union\W+select|\bdrop\b\s+(?:table|database)\b|\bwaitfor\b\s+delay\b|\bbenchmark\b|\b(if)\b\s+\(.+?=.+?\)|\blike\b\s+'.*?'|\b(?:OR|AND)\b\s+(?:(?:(?:\d+\s*[=<>]+\s*\d+)|(?:[a-z_]+\s*[=<>]+\s*[a-z_]+))|(?:\w+\s*=\s*[']?\w+[']?))|['\"]+--|(\s+and\s+\S+\s*=\s*\S+\s+and\s+'|\s+or\s+\S+\s*=\s*\S+\s+or\s+'|\s+or\s+\S+\s*=\s*\S+--\s+|\b(?:OR|AND|HAVING)\b\s*[^&?'\"/]*\S+\s*(?:=|<>)\s*[^'\"]?\S+[^'\"])", flags=re.IGNORECASE)
...
remote_os_command_injection_payload_pattern = re.compile(r'\b(echo|ping|sleep|uname|del|rm|ls|whoami|whois|exit|telnet|nc|curl|wget|bash|ruby|python|perl|netstat|exec|tftp|ftp|ver|&rem|system|shutdown)\b|(sleep|uname|del|rm\b)|\|?\b(id\s*=\s*[^&\s]*)|(^[^&\s]*\s+id\s*=\s*[^&\s]*)|(;|\||&|\n)\s*id\s*($|\||&|\s|;)|(^|\b|\n|\|n|;)(id)\b([^;\n]*)', flags=re.IGNORECASE)

#remote_os_command_injection_payload_pattern = re.compile(r'\b(echo|ping|sleep|uname|del|rm|ls|whoami|whois|exit|telnet|nc|curl|wget|bash|ruby|python|perl|netstat|exec|tftp|ftp|ver|&rem|system|shutdown)\b|(sleep|uname|del|rm\b)|\|?\b(id\s*=\s*[^&\s]*)|(^[^&\s]*\s+id\s*=\s*[^&\s]*)|(;|\||&)\s*id\s*($|\||&|\s)|(^|\b)(id)\b([^;\n]*)', flags=re.IGNORECASE) #loc file.exe
